            ...
            <div style={{display:"grid",gap:6}}>
              ...
              {mode==="owned"&&machine.ownedRates&&(
                <div>
                  ...
                  {(function(){
                    var stRate = cStones!==null?cStones:machine.ownedRates.stones;
                    var avgRateKm = stoneRows.length>0
                      ...
                      : null;
                    var impliedEurMl = (avgRateKm!==null && stonePriceEur!==null)
                      ? (avgRateKm/1000)*stonePriceEur*((CURRENCIES[currency]||CURRENCIES.EUR).rate)
                      : null;
                    return (
                      <div style={{fontSize:10,color:cl.teal,marginBottom:8,paddingLeft:148}}>
                        For 1 km: <b>{(stRate*1000).toFixed(0)} EUR</b>
                        {impliedEurMl!==null&&(
                          <span style={{marginLeft:12,color:impliedEurMl>stRate*1.2||impliedEurMl<stRate*0.8?"#f87171":cl.teal}}>
                            {" | "}Implied from stone price: <b>{impliedEurMl.toFixed(2)} EUR/ml</b>
                            {impliedEurMl>stRate*1.2&&<span style={{color:"#f87171",marginLeft:6}}>(preset may be underestimated)</span>}
                            {impliedEurMl<stRate*0.8&&<span style={{color:"#f87171",marginLeft:6}}>(preset may be overestimated)</span>}
                            {impliedEurMl>=stRate*0.8&&impliedEurMl<=stRate*1.2&&<span style={{color:cl.teal,marginLeft:6}}>(consistent)</span>}
                          </span>
                        )}
                        {impliedEurMl===null&&avgRateKm!==null&&(
                          <span style={{marginLeft:12,color:"#4a6a74"}}>Set stone price below to see implied EUR/ml</span>
                        )}
                      </div>
                    );
                  })()}
...
export default function App() {
  ...
  const [sharedCurrency,setSharedCur] = useState("EUR"); // shared across CostPanel and GrindPanel // full p object when custom rates set
              ...
              {asr&&(
                <div>
                  ...
                  <div style={{background:"rgba(0,0,0,0.2)",borderRadius:12,padding:18,border:"1px solid rgba(125,211,200,0.1)",marginBottom:14}}>
                    ...
                    {ctab==="cost"&&<CostPanel simResult={result} horizon={horizon} initRegion={replRegion} initOvhdPct={replOvhdPct} initWeldType={replWeldType} initJointSp={replJointSp} initCurrency={sharedCurrency} onCurrencyChange={setSharedCur} onParamsChange={function(p){if(p.region!==undefined)setReplRegion(p.region);if(p.ovhdPct!==undefined)setReplOvhd(p.ovhdPct);if(p.weldType!==undefined)setReplWeld(p.weldType);if(p.jointSp!==undefined)setReplJoint(p.jointSp);if(p.customP!==undefined)setReplCustomP(p.customP);}}/>}
                    {ctab==="grind"&&<GrindPanel simResult={result} horizon={horizon} context={context} initMachine={grindMachine} initMode={grindMode} initRegion={grindRegion} initNight={grindNight} initDist={grindDistKm} initCurrency={sharedCurrency} onCurrencyChange={setSharedCur} onParamsChange={function(p){
