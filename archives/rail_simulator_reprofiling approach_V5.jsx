...
function runSim(params) {
  ...
  var reprRemL   = params.reprRemL !== undefined ? params.reprRemL : 3.0;
  ...
  var results=params.segments.map(function(seg){
    ...
    var minResV=(MIN_RES_V[seg.railGrade]||3.0); // min vertical reserve (literature-calibrated)
    var minResL=(MIN_RES_L[seg.railGrade]||3.0); // min lateral reserve (literature-calibrated)
    ...
    for(var y=1;y<=params.horizonYears;y++){
      ...
      if(reprActive&&wL>=reprThreshFrac*limits.l){
        wL=Math.max(0,wL-reprRemL); resL-=reprRemL; // consume lateral reserve
        wV=Math.max(0,wV-reprRemV); res-=reprRemV;  // consume vertical reserve
        rcf=Math.max(0,rcf*(1-reprRcfR));
        pgLeft=gi*0.70; // restored profile reduces future wear (better than grind: 0.70 vs 0.75)
        reprCnt++; reprFlag=true;
      }
      var ground=false;
      ...
      data.push({year:y,mgt:+totMGT.toFixed(2),wearV:+Math.min(wV,limits.v).toFixed(3),wearL:+Math.min(wL,limits.l).toFixed(3),rcf:+Math.min(rcf,1).toFixed(3),res:+Math.max(0,res).toFixed(2),resL:+Math.max(0,resL).toFixed(2),ground:ground?1:0,repl:repl?1:0});
      ...
      <div style={{overflowY:"auto",maxHeight:680,paddingRight:8}}>
        ...
        <div style={{marginBottom:12}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <Sel value={currency} onChange={function(v){setCurrency(v);if(props.onCurrencyChange)props.onCurrencyChange(v);}} opts={Object.keys(CURRENCIES).map(function(k){return {v:k,l:CURRENCIES[k].label};})}/>
              ...
              {mode==="owned"&&machine.ownedRates&&(
                <div>
                  ...
                  {(function(){
                    ...
                    var stRateLocal = stRateEur * ((CURRENCIES[currency]||CURRENCIES.EUR).rate); // in local currency
                    var localSym = (CURRENCIES[currency]||CURRENCIES.EUR).symbol;
...
var HELP=[
  ...
  {id:"radius",title:"Radius Bands and Wear Factors",
   ...
   details:[
     ...
     {table:[["Band","Radius","f_V","f_L","Dominant mechanism"],
             ...
             ["r5","R > 800 m (tangent)","1.0","1.0","Pure rolling - reference baseline"]]},
  ...
  {id:"reprofiling",title:"Reprofiling Strategy",
   body:"DEFINITION: Reprofiling restores the full transversal rail profile -- crown (vertical) AND gauge face (lateral). Unlike grinding which targets the crown only, reprofiling removes metal in both V and L directions.\n\nTRIGGER: Geometry-driven, not MGT-scheduled. Initiated when lateral wear reaches 50-70% of the regulatory limit (IHHA 2019).\n\nMETAL REMOVAL: 0.5-1.0mm vertical + 2-4mm lateral per intervention.\n\nGRINDING INTERACTION: Reprofiling restores the crown, making the next scheduled grinding pass redundant. Grinding interval reset from date of reprofiling (Network Rail NR/L2/TRK/001).",
   links:[
...
function ComparePanel(props) {
  ...
  function runComparison() {
    ...
    setRun(true);
    try {
      var reprParams = {reprActive:props.reprActive,reprThresh:props.reprThresh,reprRemL:props.reprRemL,reprRemV:props.reprRemV,reprRcfR:props.reprRcfR,reprSkip:props.reprSkip!==false};
    ...
    <div style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,0.8)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",padding:40}}>
      <div style={{background:"linear-gradient(160deg,#0d1f2a,#0a1820)",border:"1px solid rgba(125,211,200,0.2)",borderRadius:16,width:"100%",maxWidth:900,maxHeight:"85vh",display:"flex",flexDirection:"column",boxShadow:"0 32px 80px rgba(0,0,0,0.6)"}}>
        ...
        <div style={{padding:"10px 24px",borderTop:"1px solid rgba(125,211,200,0.08)",fontSize:11,color:"#3a5a64",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
          <span>v1.1 - EN 13674 / UIC 714 / Infrabel 2023 / Guangzhou 2021 - Created by <b style={{fontWeight:700}}>Mohamed BOUDIA</b></span>
...
function getRate(code, liveRates, customRates) {
...
export default function App() {
  ...
  // Fetch on mount via useState initializer (runs once, no extra hooks needed)
  useState(function(){ setTimeout(function(){ fetchRates(); }, 300); return null; });
  ...
  var run=useCallback(function(){
    ...
    try{setErr(null);var r=runSim({context:context,trains:trains,segments:allSegs,strategy:strategy,railType:railType,trackMode:trackMode,speed:speed,lubrication:lubr,horizonYears:horizon,customLimV:customLimActive?customLimV:null,customLimL:customLimActive?customLimL:null,reprActive:reprActive,reprThresh:reprThresh,reprRemL:reprRemL,reprRemV:reprRemV,reprRcfR:reprRcfR,reprSkip:reprSkip});setRes(r);setAi(0);setHR(true);}
    catch(e){setErr("Simulation error: "+e.message);}
  },[context,trains,segs,strategy,railType,trackMode,speed,lubr,horizon,isBF,initCond,specialZones,customLimActive,customLimV,customLimL,reprActive,reprThresh,reprRemL,reprRemV,reprRcfR,reprSkip]);
      ...
      {showRatesPop&&(
        <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.5)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={function(){setShowRatesPop(false);}}>
          <div onClick={function(e){e.stopPropagation();}} style={{background:"#1a2f38",borderRadius:12,padding:24,width:460,maxWidth:"95vw",maxHeight:"80vh",overflowY:"auto",border:"1px solid rgba(125,211,200,0.2)"}}>
            ...
            <div style={{display:"grid",gap:6}}>
              {Object.keys(CURRENCIES).filter(function(k){return k!=="EUR";}).map(function(code){
            ...
            {reprActive&&(
              <div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
                  ...
                  <div>
                    <Lbl>Lateral removal per intervention (mm)</Lbl>
                    <Inp value={reprRemL} onChange={function(v){setReprRemL(+v);}} min={0.5} max={8} step={0.5}/>
                    <div style={{fontSize:10,color:cl.dim,marginTop:3}}>Gauge face metal removed per reprofiling</div>
                  </div>
                  <div>
                    <Lbl>Vertical removal per intervention (mm)</Lbl>
                    <Inp value={reprRemV} onChange={function(v){setReprRemV(+v);}} min={0.1} max={3} step={0.1}/>
                    <div style={{fontSize:10,color:cl.dim,marginTop:3}}>Crown metal removed -- consumes grinding reserve</div>
                  </div>
                  <div>
            ...
            <div>
              ...
              {asr&&(
                <div>
                  ...
                  <div style={{display:"flex",gap:6,marginBottom:12}}>
                    {[["wear","Wear V and L"],["rcf","RCF Index"],["reserve","Metal Reserve"],["plan","Schedule"],["cost","Replacement Cost"],["grind","Grinding Cost"],["repr","Reprofiling Cost"],["cmp","Strategy Comparison"]].map(function(item){return <Btn key={item[0]} onClick={function(){setCt(item[0]);}} active={ctab===item[0]} sm={true}>{item[1]}</Btn>;})}
                  </div>
                    ...
                    {ctab==="reserve"&&(
                      <div>
                        <div style={{fontSize:12,color:cl.dim,marginBottom:4}}>
                          ...
                          <span style={{marginLeft:16,color:cl.warn,fontSize:10}}>- - min 2mm (V) / 1mm (L)</span>
                        </div>
                        <ResponsiveContainer width="100%" height={250}>
                          <LineChart data={asr.data}>
                            ...
                            <ReferenceLine y={2} stroke={cl.warn} strokeDasharray="4 4" label={{value:"Min V ~3mm",fill:cl.warn,fontSize:9}}/>
                            {reprActive&&<ReferenceLine y={3} stroke="#38bdf8" strokeDasharray="4 4" label={{value:"Min L ~3mm",fill:"#38bdf8",fontSize:9}}/>}
                      ...
                      <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                        ...
                        <tbody>
                          {result.results.map(function(r,i){
                            ...
                            var passes=r.data?r.data.reduce(function(a,d){return a+d.ground;},0):0;
                            var grade=r.seg.grade||r.seg.railGrade||"R260";
                            var gFx=(CURRENCIES[sharedCurrency]||CURRENCIES.EUR).rate;
                            var gSym=(CURRENCIES[sharedCurrency]||CURRENCIES.EUR).symbol;
                        ...
                        <tfoot>
                          <tr style={{borderTop:"2px solid rgba(125,211,200,0.2)",background:"rgba(125,211,200,0.05)"}}>
                            {(function(){
                              ...
                              var gFx2=(CURRENCIES[sharedCurrency]||CURRENCIES.EUR).rate;
                              var gSym2=(CURRENCIES[sharedCurrency]||CURRENCIES.EUR).symbol;
