...
var HELP=[
  {
    id:"overview",
    title:"Overview",
    body:"Rail Wear and Maintenance Simulator v1.1 - Created by Mohamed BOUDIA.\n\nPURPOSE: Estimates rail wear progression, grinding intervention cycles, replacement timelines, and full lifecycle costs (replacement + grinding + grinding stones) for tram, metro/LRT, and heavy rail infrastructure.\n\nPHYSICS ENGINE: Archard wear model (1953) + Eisenmann dynamic load formula + RCF damage accumulation model.\n\nCALIBRATION: Infrabel/TU Delft 2023 big-data study (5338 km, 2012-2019, Belgium) and Guangzhou Metro field measurements (2021-2022, China).\n\nAPPLICABLE CONTEXTS: Tram lines (Q_ref=10t), metro/LRT (Q_ref=15t), heavy rail (Q_ref=22.5t). Each radius band segment is simulated independently on an annual time step.\n\nOUTPUTS: Wear curves, RCF index, metal reserve, grinding schedule, replacement year, replacement cost, grinding cost, stone consumption.\n\nSTANDARDS: EN 13674-1:2011 (rail geometry limits), UIC 714R (rail inspection), EN 15692 (gauge corner contact), prEN 17343 (rail grinding specification)."
  },
  {
    id:"mgt",
    title:"MGT - Traffic Loading",
    body:"DEFINITION: Gross MGT/yr = (Passes/day x Axle load x Bogies x Axles/bogie x 365) / 1,000,000. Multiple train types are supported and summed linearly.\n\nEQUIVALENT MGT (damage): MGT_eq = MGT x (Q_axle / Q_ref)^3. The cube law means a 20t axle causes ~8x more wear damage than a 10t axle at the same gross tonnage. Exponent n=3 for wear (Archard), n=4 for fatigue/RCF.\n\nPARAMETERS AND IMPACTS:\n- Passes/day: direct linear effect on MGT. Doubling passes = doubling wear rate.\n- Axle load: cubic effect via equivalent MGT. Most sensitive parameter - a 10% increase in axle load increases wear by ~33%.\n- Bogies x Axles/bogie: linear effect, total axles per train.\n\nREFERENCE AXLE LOADS: Tram=10t, Metro=15t, Heavy rail=22.5t (standard mixed freight/passenger).\n\nSOURCES: Archard J.F. (1953) Contact and Rubbing of Flat Surfaces, J. Applied Physics 24(8). IHHA Wheel-Rail Interface Guidelines, 5th ed. (2019). UIC Leaflet 714R - Rail defects (2004). EN 13674-1:2011 Annex A - load equivalence."
  },
  {
    id:"radius",
    title:"Radius Bands and Wear Factors",
    body:"CONCEPT: Track is divided into 5 radius bands. Each band has a vertical wear multiplier (f_V) and a lateral wear multiplier (f_L) relative to tangent track (band r5, R>=800m, f_V=1.0, f_L=1.0).\n\nBAND TABLE (f_V / f_L):\n- r1: R<100m       f_V=6.0  f_L=15.0 - Very tight curves, severe flange/gauge face contact\n- r2: R100-200m    f_V=4.0  f_L=9.0  - Tight curves, significant lateral creep\n- r3: R200-400m    f_V=2.5  f_L=5.0  - Moderate curves, mixed wear modes\n- r4: R400-800m    f_V=1.5  f_L=2.5  - Gentle curves, mostly crown contact\n- r5: R>=800m      f_V=1.0  f_L=1.0  - Tangent reference\n\nHOW TO USE: Enable the bands present on your line. Set the representative radius within each band (e.g. R300 for a curve in the r3 band). Lateral wear is additionally reduced by the lubrication level. Rail grade (hardness) is set per band - tighter curves benefit less from harder grades.\n\nIMPACT: f_L of 15 on r1 means lateral wear is 15x the tangent rate before lubrication. Choosing the wrong representative radius within a band has a moderate effect (5-15%); choosing the wrong band has a major effect (50-300%).\n\nSOURCES: Infrabel/TU Delft (2023) Big-data analysis of rail wear, Wear 522. Magel E. et al. (2017) Wheel-Rail Tribology, Elsevier. EN 13231-3:2012 (inspection and acceptance of rail grinding). SNCF referentiel VB 0.06 - usure des rails (2018)."
  },
  {
    id:"wear",
    title:"Wear Rate Model",
    body:"BASE RATE: 0.82 mm/100MGT for vertical crown wear on R260 grade, tangent track, standard ballasted formation, speed 80 km/h. Source: Infrabel/TU Delft 2023 statistical analysis of 5338 km.\n\nFULL FORMULA:\nwearRate_V = 0.82 x f_V(band) x hardnessEffect x f_railType x f_trackForm x f_speed\nwearRate_L = 1.00 x 1.5 x f_L(band) x hardnessEffect x f_railType x f_trackForm x f_speed x f_lubrication\n\nPARAMETER IMPACTS:\n- Rail grade hardness: R400HT saves ~38% wear on tangent but only ~14% on R<150m (hardness effect capped). Formula: he = 1 - (1-f_wear)/(1 + f_L*0.3). Choosing a harder grade is most effective on tangent and gentle curves.\n- Rail type: Groove rail adds ~20% vertical and 80% lateral vs vignole (head contact geometry).\n- Track form: Slab track +10-15% vs ballasted (higher rigidity increases dynamic loads). Embedded tram track +15-20%.\n- Speed: Below 40 km/h, vertical wear is 10% lower (less dynamic amplification). Above 120 km/h, vertical wear increases 10-35%.\n\nSOURCES: Infrabel/TU Delft (2023) 10.1016/j.wear.2022.204764. Esveld C. (2001) Modern Railway Track, 2nd ed. Magel E. (2011) Rolling Contact Fatigue: A Comprehensive Review, NRCC. EN 13674-1:2011 - rail profiles and wear limits."
  },
  {
    id:"rcf",
    title:"RCF - Rolling Contact Fatigue",
    body:"DEFINITION: RCF (Rolling Contact Fatigue) is surface and sub-surface crack initiation due to cyclic plastic deformation at the wheel-rail contact. The RCF index (0 to 1) represents accumulated damage relative to the failure threshold.\n\nRCF PARADOX - MAGIC WEAR RATE: Moderate curves (r4, R400-800m) accumulate HIGHER RCF than tight curves (r1). Why? Tight curves wear fast enough to continuously remove the surface crack layer before cracks propagate (self-healing). Moderate curves have enough contact stress to initiate cracks but too little wear to remove them.\n\nRCF INDEX THRESHOLDS:\n- 0.0 to 0.3: Healthy - preventive grinding is sufficient to maintain the profile\n- 0.3 to 0.7: Moderate - corrective grinding required, increasing number of passes\n- 0.7 to 1.0: Critical - replacement mandatory, cracks too deep for grinding\n\nFORMULA: RCF increment/yr = rcfBase x MGT x (1 - wear_protection). Wear protection = min(0.80, wearRate/5.0). After grinding, RCF is reduced by passes x rcfReduction x health factor.\n\nIMPACT: Allowing RCF to reach 0.7 before intervention doubles the number of grinding passes needed vs maintaining below 0.3. Above 0.7, replacement cost is triggered even if wear limits are not reached.\n\nSOURCES: Infrabel/International Journal of Fatigue (2025) Analysis of 212 instrumented curves. Squires G. et al. (2006) Rolling Contact Fatigue, RailwayResearch. UIC 712R - Rail defect catalogue (2002). Ringsberg J.W. (2001) Life prediction of rolling contact fatigue crack initiation, Int. J. Fatigue 23(7)."
  },
  {
    id:"grinding",
    title:"Grinding Strategy",
    body:"TWO STRATEGIES MODELLED:\n\nPREVENTIVE: Frequent short interventions before significant RCF or profile loss.\n- Interval: base table per band and context (e.g. 20 MGT for metro r4, 80 MGT for heavy r5)\n- Removal per pass: 0.20 mm\n- Post-grinding wear factor: 0.75 (restored profile reduces future wear by 25%)\n- RCF reduction per pass: ~30% with health bonus\n- Result: RCF kept below 0.3, rail life 400-600 accumulated MGT before replacement\n\nCORRECTIVE: Threshold-triggered interventions when profile or RCF has degraded.\n- Interval: 3x longer than preventive base\n- Removal per pass: 0.55 mm, up to 4 passes\n- Post-grinding wear factor: 0.92 (partial profile restoration)\n- RCF reduction per pass: ~18%\n- Result: Rail life 200-350 MGT, higher metal consumption per intervention\n\nPARAMETER IMPACT: The strategy choice is the second most impactful parameter after axle load. Preventive extends rail life by 50-100% compared to corrective. The difference is driven by the post-grinding wear factor: a restored profile generates less dynamic load amplification.\n\nSOURCES: BNSF Railway preventive-gradual grinding program (2018). Delhi Metro Rail Corp O&M plan, Phase III (2020). Infrabel Grinding Management Report (2022). Grassie S.L. (2005) Rail corrugation: advances in measurement, understanding and treatment, Wear 258."
  },
  {
    id:"lubrication",
    title:"Flange Lubrication",
    body:"FUNCTION: Reduces friction at the flange/gauge face contact. Affects LATERAL wear only - no effect on crown (vertical) wear.\n\nLUBRICATION EFFECTIVENESS BY BAND (factor applied to lateral wear rate):\n- No lubrication:     1.00 / 1.00 / 1.00 / 1.00 / 1.00 (r1 to r5)\n- Poor/badly maintained: 0.80 / 0.82 / 0.88 / 0.95 / 1.00\n- Standard wayside:   0.55 / 0.60 / 0.72 / 0.90 / 1.00\n- Good (wayside+onboard): 0.35 / 0.40 / 0.60 / 0.85 / 1.00\n- Optimal (lab only): 0.10 / 0.15 / 0.35 / 0.75 / 1.00\n\nIMPACT: Standard wayside lubrication reduces lateral wear by 45% on R<100m curves, 40% on R100-200m, 28% on R200-400m, and has no effect on tangent. No effect on RCF (lubrication does not affect contact stress distribution at crown). Lateral wear is typically the limiting factor on curves below R200m.\n\nWARNING: Optimal level is laboratory conditions only. In revenue service, oil contamination, rain, leaf fall, and maintenance intervals make Good the practical maximum.\n\nSOURCES: Swedish SJ field trials on lateral wear with friction modifiers (2018), Banverket report. Shanghai Metro Line 2 lateral wear study (2021), Journal of Rail and Rapid Transit. Arias-Cuevas O. et al. (2010) Rolling-sliding laboratory tests of friction modifiers in dry and wet conditions, Wear 268."
  },
  {
    id:"brownfield",
    title:"Brownfield Mode",
    body:"PURPOSE: Allows simulation starting from existing worn rail instead of new rail (greenfield). Essential for inherited projects, condition assessments, and remaining life evaluations.\n\nINPUT PARAMETERS (per active segment):\n- Vertical wear (mm): measured worn depth from original profile height. Impact: simulation starts at this value; replacement triggered sooner if close to limit.\n- Lateral wear (mm): measured gauge face wear. Often the critical dimension on tight curves.\n- RCF index (0.0 to 1.0): field assessment or ultrasonic measurement. Impact: if above 0.3, corrective grinding is immediately scheduled in year 1.\n- Accumulated MGT: total tonnage since rail installation. Used for lifecycle tracking and total cost amortisation.\n\nHEALTH INDICATOR: health = max(wearV/limitV, wearL/limitL, RCF). Good < 40%, Moderate 40-70%, Poor > 70%. The bar shows percentage of service life already consumed.\n\nMETAL RESERVE ADJUSTMENT: Initial reserve = nominal reserve - (measured wearV x 0.8). The 0.8 factor accounts for the fact that grinding passes also consume reserve; field-measured vertical wear underestimates total consumed reserve.\n\nTYPICAL USE: Input last inspection report values. The simulator immediately shows: years remaining, whether a corrective grinding pass is needed before the next scheduled intervention, and updated replacement budget.\n\nSOURCES: EN 13231-1:2016 (acceptance of railway track geometry). Network Rail NR/SP/TRK/001 - Track inspection handbook (2021). SNCF Reseau DIN 00122 - Etat initial du rail pose (2019)."
  },
  {
    id:"replacement",
    title:"Replacement Criteria",
    body:"REPLACEMENT IS TRIGGERED when ANY ONE of four conditions is met:\n\n1. VERTICAL WEAR >= limit:\n   - Tram: 7mm | Metro: 9mm | Heavy rail: 12mm\n   - Measured from original rail head height. Reduces structural section and increases stress concentration.\n\n2. LATERAL WEAR >= limit:\n   - Tram: 8mm | Metro: 11mm | Heavy rail: 14mm\n   - Measured at 14mm below running surface per EN 13674-1. Affects gauge and flange clearance.\n\n3. METAL RESERVE <= 2mm:\n   - Initial reserves: R200=13mm, R260=15mm, R320Cr=16mm, R350HT=17mm, R400HT=18mm\n   - Consumed by each grinding pass (0.20mm preventive, 0.55mm corrective per pass)\n   - 2mm safety threshold ensures structural integrity of the remaining head\n\n4. RCF INDEX >= 0.70:\n   - At this level, cracks have typically propagated 5-8mm below the surface\n   - Grinding cannot reach this depth without exceeding the metal reserve\n   - Immediate replacement required regardless of wear measurements\n\nIMPACT OF STRATEGY: Preventive grinding preserves metal reserve (0.20mm/pass x 1 pass) while corrective grinding consumes it rapidly (0.55mm/pass x up to 4 passes = 2.2mm per intervention). For R350HT with 17mm reserve and corrective strategy, only 6-7 interventions are possible before reserve is exhausted.\n\nSOURCES: EN 13674-1:2011 Table 2 (wear limits). UIC 714R - Rail defect classes and action levels (2004). Network Rail NR/L2/TRK/001 (2022). Infrabel TR - Inspection frequencies and renewal criteria (2021)."
  },
  {
    id:"cost_repl",
    title:"Replacement Cost Estimation",
    body:"SCOPE: Estimates total cost of track renewal (both rails) per linear meter and per segment.\n\nSIX COST COMPONENTS:\n1. Labour: fully loaded team rates (salary + social charges + overhead). Team = 1 foreman + 4 technicians + 2 welders + 2 machinists (adjustable). Unit: EUR/h per person.\n2. Rail material: price per tonne x kg/m x 2 rails. R260 grade weighs 60 kg/m; R400HT also 60 kg/m (similar section). Premium grades cost 20-40% more per tonne.\n3. Equipment rental: tamping machine, rail-road vehicle, track crane, logistics truck. Optional: pre-grinding pass before relaying (improves track geometry acceptance).\n4. Welding: cost per joint x joints per meter (1/joint spacing). Aluminothermic (thermit) is standard on most networks; flash butt is more expensive but produces higher quality joints (used on high-speed lines).\n5. Tooling and consumables: 5% of labour cost (rail anchors, fishplates, clips, bolts).\n6. Overhead and supervision: configurable %, typically 15-22% of direct costs.\n\nREGIONAL PRESETS: Western Europe, Eastern Europe, North Africa/Middle East, Sub-Saharan Africa, South-East Asia, Latin America. Labour rates vary by factor of 5-8x between regions; material varies by 30-70% (import premium for remote regions).\n\nNIGHT PRODUCTIVITY: 70% efficiency factor applied to daytime productivity rates, accounting for reduced visibility, fatigue, and safety constraints.\n\nSOURCES: RFI (Italy) contractor rate database 2023. Infrabel (Belgium) budget reference costs 2022. ONCF (Morocco) contractor rates 2022. World Bank Railway Toolkit - Unit costs for rail renewal (2019). AREMA Manual for Railway Engineering Ch.4 (2022)."
  },
  {
    id:"cost_grind",
    title:"Grinding Cost Estimation",
    body:"SCOPE: Estimates total cost of all grinding interventions over the simulation horizon.\n\nTHREE MACHINE TYPES:\n- Small machine (tram/metro): 16-24 grinding heads, ~200 ml/h effective speed, suitable for light rail, metro curves, depot tracks.\n- Line machine (ballasted track): 32-48 heads, ~400 ml/h, standard for suburban and mainline networks with their own fleet.\n- Specialist machine (Speno/Loram/Vossloh): 64-120 heads, ~800 ml/h, subcontract only, cost-effective for large networks (>100 km/yr).\n\nOWN FLEET MODE - Cost components per ml per pass:\n- Grinding stones: EUR/ml (see Grinding Stones section for details)\n- Fuel/energy: diesel or electric traction cost\n- Vehicle maintenance: depreciation, periodic servicing\n- Labour: team of 2-3 operators at regional rates\n\nSUBCONTRACT MODE - Cost components:\n- Operation rate: EUR/ml/pass (includes all consumables, labour, machine cost)\n- Mobilisation fixed fee: travel logistics, insurance, documentation (EUR/intervention)\n- Mobilisation per km: from contractors depot to site (EUR/km)\n- Mobilisation options: per intervention (spot contracts) vs once per horizon (framework contracts)\n\nKEY INSIGHT: For small networks (<30 km), subcontracting is almost always cheaper due to high mobilisation costs of owning and maintaining a grinding machine. For large networks (>100 km with frequent grinding), own fleet becomes competitive.\n\nSOURCES: Speno International SA - Grinding services technical brochure (2022). Loram Technologies - Rail grinding effectiveness report (2021). RFI (Italy) grinding contract data 2022. Infrabel (Belgium) grinding budget reference 2023. Zarembski A.M. (2005) The art and science of rail grinding, AREMA proceedings."
  },
  {
    id:"stones",
    title:"Grinding Stone Consumption",
    body:"FUNCTION: Grinding stones (also called grinding wheels or abrasive stones) are the consumable elements mounted on the grinding heads. Each head carries one stone. They wear during the grinding operation and must be replaced periodically.\n\nCONSUMPTION FACTORS:\n1. Radius (most important): Tight curves (R<100m) require 5-8x more stones per km than tangent track because higher contact angles cause faster stone wear. Base rates per km per rail per pass: r1=2.0-5.0, r2=1.25-3.25, r3=0.75-1.75, r4=0.50-1.10, r5=0.40-1.00 (range = small to Speno machine).\n2. Rail grade hardness: Harder rails wear the stones faster. Grade factors: R260=1.0x, R320Cr=1.15x, R350HT=1.30x, R400HT=1.45x. Choosing R400HT on a heavy-traffic tangent increases stone consumption by 45%.\n3. Machine type: Larger machines with more heads and higher grinding pressure consume more stones per km but grind faster.\n4. Pass depth: Not explicitly modelled (rates are calibrated for standard pass depths).\n\nSTONE WEIGHT: Small machine ~0.9 kg/stone, line machine ~1.4 kg/stone, Speno/Loram ~2.2 kg/stone.\n\nCUSTOM RATE: Enter your measured or contractor-quoted rate for the R200-400m band. All other bands scale proportionally from the preset ratio.\n\nCOST: If unit price is entered (typical range 5-20 EUR/stone depending on abrasive type, supplier, and quantity), total stone cost is computed per segment and globally. Stone cost typically represents 30-50% of own-fleet grinding cost.\n\nSOURCES: Speno International technical bulletin TB-2019-04 (stone consumption factors). Loram Technologies field data summary (2021). Vossloh Rail Services application guide for grinding wheels (2020). RAME I. et al. (2018) Abrasive wear of grinding wheels in rail grinding, Wear 406-407."
  },
  {
    id:"validation",
    title:"Validation and Calibration",
    body:"PURPOSE: Compares simulator predictions against real-world field measurements to verify the model is calibrated for your specific context.\n\nHOW IT WORKS: Predictions are computed using the FULL simulation engine with your current parameters (rail type, track form, speed, lubrication, strategy) - not a simplified sub-model. A synthetic train fleet is built to reproduce the reference case MGT/yr.\n\nREFERENCE CASES INCLUDED:\n- BE1: Infrabel/TU Delft 2023, heavy rail, tangent, R260, 25 MGT/yr. Measured V=0.82 mm/100MGT. This is the model calibration baseline.\n- BE2: Same source, R500m curve, R260, 25 MGT/yr. Measured V=1.40, L=2.80 mm/100MGT. Tests the curve wear factor model.\n- BE3: Same source, tangent, R200 grade, 25 MGT/yr. Measured V=1.10 mm/100MGT. Tests the hardness model on tangent.\n- GZ1: ScienceDirect Wear 2021, Guangzhou Metro, R300m, R260, 15 MGT/yr. Measured V=2.10, L=6.50 mm/100MGT. Tests metro context with tight curves.\n- GZ2: Railway Sciences 2022, EMU depot, R350m - INCOMPARABLE. Published value (10.1mm lateral) is absolute wear after 1 million passes, not a mm/100MGT rate. Marked with ! to prevent false comparison.\n\nDEVIATION THRESHOLDS: Green <15% (good calibration), Yellow 15-30% (acceptable, reconsider lubrication or speed), Red >30% (recalibrate - likely context-specific factors not captured).\n\nADDING YOUR OWN DATA: Click Add real measurement to enter field measurements. Enter the representative radius, rail grade, annual MGT, and measured wear rates from inspection reports. The simulator instantly computes the prediction and shows deviation.\n\nWHEN TO RECALIBRATE: If deviation >30% on multiple cases from your context, consider adjusting the base wear rate (contact the developer). Common causes: different wheel profile geometry, unusual track form, specific rail steel chemistry.\n\nSOURCES: Rooij L. et al. (2023) Statistical analysis of rail wear on Belgian network, Wear 522. Liu B. et al. (2021) Field investigation of rail wear on Guangzhou Metro, ScienceDirect Wear 477. Wang W.J. et al. (2022) Wear of EMU depot track, Railway Sciences 1(2). ASTM E2660 - Standard for wear measurement in railway track."
  },
  {
    id:"limits",
    title:"Known Limitations",
    body:"VERSION 1.1 - Annual time step simulation.\n\nNOT MODELLED (known gaps):\n- Inner vs outer rail asymmetry within a curve (outer rail is always the critical one; inner rail wears at approximately 30-60% of the outer rail rate on curves - add as a separate segment with adjusted radius if needed).\n- Wheel profile evolution over time (new wheels are conical, worn wheels develop hollow tread which shifts contact to gauge corner).\n- Seasonal variation: leaf fall increases wear by 15-25% in autumn (abrasive contamination); wet conditions in winter reduce friction and alter wear mode.\n- Station braking and acceleration zones: wear can be 2-4x higher in these zones. Workaround: add as a short separate segment (50-200m) with f_V multiplied by 1.5-3.0.\n- Switch and crossing wear: totally different wear mechanisms, not applicable.\n- Corrugation: short-pitch (30-80mm wavelength) roughness development requires separate dynamic model.\n- Sub-annual loading peaks (commuter peak hours vs off-peak).\n\nCALIBRATION SCOPE: Primarily European heavy rail (Infrabel Belgium) and Chinese metro (Guangzhou). For other contexts (North American freight, Japanese Shinkansen, African narrow gauge), use the Validation tab to add local measurements and assess deviation before using for budget decisions.\n\nCOST DATA: All rates are based on 2022-2023 data. Update with local contractor quotes before any budget submission. Exchange rates are indicative fixed values and should be updated for actual financial planning.\n\nSOURCES: CONTACT the developer (Mohamed BOUDIA) for context-specific calibration. Repository of rail wear databases: RSSB T1009 (UK), FRA Track Safety Standards (USA), DB Netz Schienenverschleiss (Germany)."
  },
];
