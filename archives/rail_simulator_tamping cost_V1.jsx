...
function TampingCostPanel(props) {
  ...
  var tcp     = calcTampCostPerMl(machine, mode, region, nightHrs, currencyMap, currency);
    ...
    <div style={{display:"grid",gap:16}}>
      ...
      <Card title="Machine and Rates">
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:12}}>
          <div>
            ...
            <div style={{display:"flex",gap:6}}>
              {["sub","owned"].map(function(m){
                return <div key={m} onClick={function(){setMode(m); resetOp();}}
          ...
          <div>
            <Lbl>Machine type</Lbl>
            <Sel value={machineKey} onChange={function(v){setMachine(v); resetOp();}}
              opts={Object.keys(TAMP_MACHINES_BOUR).map(function(k){return {v:k,l:TAMP_MACHINES_BOUR[k].label};})}/>
            <div style={{fontSize:10,color:cl2.dim,marginTop:3}}>{machine.prodMlH} m/h productivity</div>
          </div>
          <div>
            <Lbl>Region</Lbl>
            <Sel value={region} onChange={function(v){setRegion(v); resetOp(); resetDeg(); resetBall();}}
